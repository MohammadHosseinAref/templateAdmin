...
function SubSubRow({
  value, catName, subName, onEdit, onDelete,
}: {
  value: string; catName: string; subName: string; onEdit: () => void; onDelete: () => void;
}) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 transition-colors group">
      ...
      <span className="flex-1 min-w-0">
        ...
        <span className="text-xs text-slate-400 dark:text-slate-500 ms-2">← {catName} / {subName}</span>
      </span>
      {confirm ? (
...
type EditTarget = { categoryId: string; subName: string; oldValue: string } | null;

export default function SubSubTab() {
  const t = useLocale().categories;
  const { categories, addSubSub, updateSubSub, deleteSubSub } = useCategories();
  ...
  const subOptions = categories.find((c) => c.id === selectedCatId)?.subs ?? [];

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const trimmed = name.trim();
    if (!trimmed || !selectedCatId || !selectedSubName) return;
    if (editing) { updateSubSub(editing.categoryId, editing.subName, editing.oldValue, trimmed); setEditing(null); }
    else addSubSub(selectedCatId, selectedSubName, trimmed);
    setName('');
  }

  const isEditing = editing !== null;

  const allSubSubs = categories.flatMap((cat) =>
    cat.subs.flatMap((sub) =>
      sub.subs.map((ss) => ({ catId: cat.id, catName: cat.name, subName: sub.name, value: ss })),
    ),
  );
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="w-full bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
        ...
        <div className="flex gap-3">
          <select
            value={selectedCatId}
            onChange={(e) => { setSelectedCatId(e.target.value); setSelectedSubName(''); }}
            disabled={isEditing}
          ...
          <select
            ...
            disabled={isEditing || !selectedCatId}
            className="h-10 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 px-3 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all disabled:opacity-60"
          >
            <option value="">{t.fields.selectSub}</option>
            {subOptions.map((sub) => (
          ...
          <input
            ...
            placeholder={t.placeholders.subSubName}
          ...
          />
          <button type="submit" disabled={!name.trim() || !selectedCatId || !selectedSubName} className="h-10 px-6 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed" style={{ backgroundColor: '#0d9488' }}>
      ...
      <div className="space-y-2">
        {filtered.length === 0
          ...
          : filtered.map(({ catId, catName, subName, value }) => (
              <SubSubRow
                key={`${catId}::${subName}::${value}`}
                value={value}
                catName={catName}
                subName={subName}
                onEdit={() => { setEditing({ categoryId: catId, subName, oldValue: value }); setSelectedCatId(catId); setSelectedSubName(subName); setName(value); }}
                onDelete={() => deleteSubSub(catId, subName, value)}
              />
            ))
        }
      </div>
