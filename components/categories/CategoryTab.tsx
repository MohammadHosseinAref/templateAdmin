...
function CategoryRow({
  id, name, onEdit, onDelete,
}: {
  id: string; name: string; onEdit: () => void; onDelete: () => void;
}) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 transition-colors group">
      <span className="w-2 h-2 rounded-full flex-shrink-0 bg-teal-400" />
      <span className="flex-1 text-sm font-medium text-slate-800 dark:text-slate-100">{name}</span>
      {confirm ? (
...
export default function CategoryTab() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!trimmed) return;
    if (editingId) { updateCategory(editingId, trimmed); setEditingId(null); }
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="w-full bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
        ...
        <div className="flex gap-3">
          ...
          />
          <button type="submit" disabled={!name.trim()} className="h-10 px-6 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed" style={{ backgroundColor: '#0d9488' }}>
            {isEditing ? t.labels.save : t.labels.add}
          </button>
          {isEditing && (
            <button type="button" onClick={() => { setEditingId(null); setName(''); }} className="h-10 px-4 rounded-xl text-sm font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-600 transition-colors">
              {t.labels.cancel}
            </button>
          )}
        </div>
        ...
        {filtered.length === 0
          ...
          : filtered.map((cat) => (
              <CategoryRow
                ...
                id={cat.id}
                name={cat.name}
                onEdit={() => { setEditingId(cat.id); setName(cat.name); }}
